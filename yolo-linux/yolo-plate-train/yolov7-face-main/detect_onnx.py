...
def post_process(x):
    ...
    anchors=[  
            [ 41,14,  101,31,  191,58],
            [275,88,  380,114,  361,169] ,
            [ 501,146,  551,195,  510,316]
    ]
...
#   - [ 501,146,  551,195,  510,316]
    ...
    a = torch.tensor(anchors).float().view(nl, -1, 2).to(x[0].device)

    anchor_grid=a.clone().view(nl, 1, -1, 1, 1, 2)  # shape(nl,1,na,1,1,2)

    for i in range(nl):

        # x[i] = torch.cat((im[i](m[i](ia[i](x[i]))),m_kpt[i](x[i])), axis=1)
        ...
        if inplace:
            ...
            if nkpt != 0:
                x_kpt[..., 0::3] = (x_kpt[..., ::3] * 2. - 0.5 + kpt_grid_x.repeat(1,1,1,1,nkpt)) * stride[i]  # xy
                x_kpt[..., 1::3] = (x_kpt[..., 1::3] * 2. - 0.5 + kpt_grid_y.repeat(1,1,1,1,nkpt)) * stride[i]  # xy
            ...
            if nkpt != 0:
                y[..., 6:] = (y[..., 6:] * 2. - 0.5 + grid[i].repeat((1,1,1,1,nkpt))) * stride[i]  # xy
...
def detect(opt):
    # net_jit=torch.jit.load(r'/disks/disk1/Workspace/TrainProjs/yolov7-face-main/runs/train/exp6/weights/best.torchscript.pt').to('cuda:0')

    # model_path=r'/disks/disk1/Workspace/TrainProjs/yolov7-face-main/runs/train/exp6/weights/best.onnx'
    model_path=[
        # r'/disks/disk1/Workspace/TrainProjs/yolov7-face-main/runs/train/exp6/weights/best-sim.onnx'
        r'/disks/disk1/Workspace/TrainProjs/yolo-plate/yolov7-face-main/runs/train/exp20/weights/last.onnx'
    ][0]
    ...
    for path, img, im0s, vid_cap in dataset:
        img_foronnx=img.astype(np.float32)/255.0
        img_foronnx = np.expand_dims(img_foronnx,0)
        ...
        # pred=net_jit(img)
        pred = onnx_session.run([], {input_name: img_foronnx})
        pred[0]=torch.from_numpy(pred[0]).to(device)
        pred[1]=torch.from_numpy(pred[1]).to(device)
        pred[2]=torch.from_numpy(pred[2]).to(device)
        pred=post_process(pred)

        # model_inference(model_path=None, input=None)
